...
//
// What this pins (AC-3.2.1/.2/.3/.4/.7):
//   - happy path: deal → every seat acts (last action → allActed) → Host revealAll → every device gets a
//     `showdown` projection with revealed===true AND every seat's hand present (SM-6-over-the-wire: the
//     reveal is the FIRST moment a non-owner receives another seat's card — the inverse of the draw test's
//     "exactly one hand key" assertion: now the hand-key count === player count).
...
// [Source: epics.md#Story 3.2; architecture.md#Phase allActed→showdown; table-server-draw.do.test.ts harness.]
...
test("AC-3.2.1/.4 (heads-up): allActed → Host revealAll → every device gets showdown with ALL hands present", async () => {
  ...
  // Host triggers the reveal, carrying the current phase token.
  host.send({ type: "revealAll", payload: { phaseToken: hostAllActed.phaseToken } });

  // Every device now sees `showdown` with revealed===true and EVERY seat's hand present.
  const hostShow = await nextPhase(host, "showdown");
  const p1Show = await nextPhase(p1, "showdown");
  for (const view of [hostShow, p1Show]) {
    expect(view.phase).toBe("showdown");
    expect(view.revealed).toBe(true);
    // SM-6-over-the-wire: EVERY seat in players[] now carries its hand (the reveal is the first moment a
    // non-owner receives another seat's card) — the inverse of the pre-reveal "no players[] hand" rule.
...
test("AC-3.2.2 (double-tap): a second revealAll carries a stale token → rejected stale-phase", async () => {
  ...
  // First reveal is accepted (bumps the phase token → showdown).
  host.send({ type: "revealAll", payload: { phaseToken: tokenAtAllActed } });
  await nextPhase(host, "showdown");
